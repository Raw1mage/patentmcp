...
## 2. patentmcp_init tool（R15.1 tool 面）

- [x] 2.1 在 `src/patent_mcp_server/patents.py` 註冊 `patentmcp_init` tool，annotations `{readOnlyHint:true, destructiveHint:false, idempotentHint:true, openWorldHint:false}`（DD-5）
...
- [x] 3.1 在 server 端 **新建** `prompts/list`+`prompts/get` handler（patentmcp 現無），加入 `patentmcp_init`（或 `usage`）entry。確認 FastMCP prompts 註冊 API；範本：docxmcp `bin/mcp_server.py`（DD-6）
...
- [x] 5.1 rebuild patentmcp container，live smoke：`patentmcp_init` tool 可呼叫回 doctrine；`prompts/get patentmcp_init` 回同源 doctrine；instructions 含 signpost
