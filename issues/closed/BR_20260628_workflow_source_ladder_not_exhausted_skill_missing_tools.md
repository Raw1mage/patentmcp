...
## 驗證 B 通過記錄 2026-07-10(端到端注入驗證 — 本 BR 最後一段)

**驗證設計**:兩輪 prompt-injection 探測子代理(task prompt 不手抄任何條款),要求逐字回報其 prompt 是否含 Exhaustion Gate 區塊。

- **對照組(patentworks 未 pin,僅 loaded)**:子代理 prompt 無注入(三題全 no)——符合 `collectFromParent` 只收 **pinned** skills 的設計(delegation-clauses.ts)。
- **實驗組(pin 後)**:子代理 prompt 實含「## ⛳ Delegation clauses (runtime-injected from the orchestrator's active skills)」+ 條款 6-9 逐字命中(雙路徑取圖 / ppubs_batch_get_claims / 回報格式 / allow_scraping 沿用主代理),且子代理識別為 hard requirements。

**結論**:opencode 25e1e74d7 注入橋端到端有效。三段全綠:
- ✅ domain-local 止血(SKILL.md 委派契約)— e361e17 + 驗證 A
- ✅ 根因(runtime 注入橋)— opencode 25e1e74d7
- ✅ 驗證 B(端到端)— 本記錄

**附帶語義發現(operational note,非 bug)**:`skill()` tool 只 `recordLoaded` 不 pin;pin 來源為 mandatory-skills sentinel / compaction anchor / 操作端 API(`POST /session/<id>/skill-layer/<name>/action`)。主代理手動 `skill("patentworks")` 而未 pin 時委派**不會**注入——orchestrator 跑 patent 委派前應確保 patentworks 處於 pinned(mandatory-skills 列表或 API pin)。

Status: **Resolved**(自 REOPENED 收斂)
