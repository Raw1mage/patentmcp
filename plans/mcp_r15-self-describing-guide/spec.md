...
- **THEN** instructions 含指向 `patentmcp_guide`（或 `prompts/get patentmcp_guide`）的 signpost 句
...
- **THEN** `patentmcp_guide` tool 與 `prompts/get` 回傳的內容隨之變更（三者同源）

## Acceptance Checks

- [ ] `patentmcp_guide` tool 可呼叫,回傳含 R15.2 四類的 doctrine
- [ ] `prompts/list` 列出 `patentmcp_guide` entry;`prompts/get patentmcp_guide` 回同源 doctrine
