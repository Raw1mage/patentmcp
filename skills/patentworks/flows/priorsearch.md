...
## 1.4 端到端主幹:從需求書到代表案(8 步流水線,使用者天條 2026-07-20)

> **這是 priorsearch 的貫穿骨架。** 鐵律 −1/−2(五節迴圈 + 精雕迴圈)管「累積粗池」那一段的鬆緊;本節把整條「需求書 → 代表案」的主線串起來,任何正式前案檢索都照此 8 步走,缺步即方法論不合格。§1.5 起的檢索強度、§2 三地鐵則、§3 流程都是本主幹某一步的細則。

**8 步主幹(順序不可顛倒;粗池→細池是硬分界):**

1. **需求分析(五維拆解)**——針對使用者提供的技術需求,拆解出五個構面,寫進 `00_campaign.md`:
   - **技術手段**(軟體 / 硬體 / 演算法)
   - **應用場景**(產品情境)
   - **產業領域**
   - **針對痛點 / 要解決的問題**
   - **期待功效**
   缺這步 = 檢索式沒有需求錨,後面全是海撈。
2. **檢索規劃**——由五維推出:**檢索關鍵字規劃** + **相關 IPC 碼拆解規劃**(IPC/CPC 分類碼是模糊涵蓋領域的錨,圈住主線題材域)。
3. **ABCD 檢索式設計(見 §1.4.1)**——用 ABCD 四軸方法論把關鍵字 + IPC 組成查詢式。
4. **迭代查詢累積「粗池」**——逐一迭代把所有規劃檢索式 + ABCD 延伸式**全跑完**(迭代紀律見 §1.4.2,非無腦展開),records 一律實撈落地 `02_pool/`(鐵律0:母數≠樣本)。**粗池 = 未去重去噪的草稿池**,含大量專利。
5. **去重去噪 → 「細池」(baseline pool)**——**⚠️ 硬分界:所有檢索式都跑完、累積成粗池,才做去重去噪,不是邊撈邊篩。** 去重到公開號級(GPSS 無家族 ID)/ 家族級(走 `epo_family`);去噪在標記層逐筆篩(recall 優先,鐵律0)。產物 = `02_pool/candidates.csv`(baseline pool)。
6. **多 label 分類**——對細池每件專利做:年份、國別、技術領域分類;**每件採多 label 標注技術**(多標籤 schema 見鐵律 −2 精雕迴圈 ③,同維度可複選 `|` 分隔)。
7. **技術功效矩陣(見 §1.4.3)**——從多 label 展開「技術手段 × 功效」二維矩陣,分群看密度熱區與空白區。
8. **相關性評估 → 代表案介紹**——將每件專利與**使用者需求書所述概念**做關聯度評分(1-5 級);分數前幾名(所有 5 級)拉出來逐一細讀,展開 §4「重點前案細部分析」章節(逐字 Claim 1 + 代表圖 + 白話技術解析四問)。

### 1.4.1 ABCD 檢索式設計方法論(四軸,使用者定義 2026-07-20)

每一條粗池檢索式由**四軸**組合而成:

| 軸 | 內容 | 角色 |
|---|---|---|
| **A = IPC** | IPC/CPC 分類碼 | 骨幹軸,圈住主線題材域;骨幹碼首輪定一次即**凍結**(精雕迴圈紀律),子碼細化合法、外域碼不得進骨幹 |
| **B = 技術軟硬手段關鍵字** | 軟/硬/演算法手段詞群 | OR 串接,`@TI,AB` 欄位限定 |
| **C = 情景應用關鍵字** | 應用場景 / 產業 / 痛點詞群 | OR 串接,與 B 群 AND 交集 |
| **D = 排除字** | 雜訊排除詞群 | `NOT (…)` 排除;分片時 NOT 群 **byte-identical 完整保留**(`¬(D1∪D2)=¬D1∩¬D2`,拆 NOT 群會靜默漏排) |

**組合式**:`(B1 OR B2…)@TI,AB and (C1 OR C2…) not (D1 OR D2…)`,掛 A 軸 IPC 錨定。長式撞 GPSS condition-length 牆時工具層自動分片(BR_20260715)。

### 1.4.2 逐步迭代擴充紀律(非無腦迭代,使用者天條 2026-07-20)

ABCD 詞群**不是一次列滿就跑,是每輪精雕滾大**——嫁接在鐵律 −1 五節迴圈與鐵律 −2 精雕迴圈上:

- **變數控制**:每輪只動一軸(A 的 IPC 子碼 / B 的手段詞 / C 的情景詞 / D 的排除詞),否則命中變化無法歸因。
- **PREDICT→OBSERVE 校準**:每條式子執行前先賭預期 total 區間與主軸;回來對照,不符即停(防無聲失敗)。
- **樣本回灌**:讀命中樣本的標題/claim/IPC,學到新手段詞→擴 B、新情景詞→擴 C、新雜訊型態→擴 D、新高頻 IPC→細化 A;知識回灌成下一輪式子,**不是憑空聯想堆同義詞**。
- **DECIDE 節點交使用者拍板**:收緊/放寬/轉向/補洞/續撈——AI 不自主判定「夠了」,每輪 append `search-journal.md` 五節日誌。
- **D 軸特別紀律**:排除字**延後到「診斷出雜訊型態」才加**,不預先猜;加 D 後抽樣驗有效樣本沒被誤殺(呼應鐵律0「去噪在標記層、recall 優先」)。

### 1.4.3 技術功效矩陣(technology-effect matrix,使用者天條 2026-07-20)

主幹第 7 步的交付物。**二維矩陣**:

- **列軸 = 技術手段**(technical means):從細池每件的 claim/abstract 抽出的手段標籤(對應多標籤的 `modality`/`action` 維度細化)。
- **欄軸 = 功效 / 解決的問題**(effect):抽出的功效標籤(對應需求分析第五維「期待功效」)。
- **每個 cell**:落在該(手段×功效)交集的專利計數 + 代表公開號;高密度 cell = 紅海、空白 cell = 潛在 whitespace。

**資料底鐵律(鐵律0 延伸)**:技術功效矩陣的手段×功效標籤**必須從 records 實體的 abstract/claim 抽**——母數計數階段無文字可抽,**不得**用 FTS 命中投影或只有 title 的池建矩陣(2026-07-20 RCA:FTS 投影池缺 abstract,矩陣無效)。標籤兩檔路徑:純規則(關鍵字比對)先標明確子集,`unlabeled` 殘餘才委派 subagent 上 AI 語意判讀(先規則後 AI)。

### 1.4.4 ⛔ 禁用本地 patentdb FTS 建池(使用者天條 2026-07-20,方法論紅線)

> **本地 sqlite FTS(`patentdb_query` 的 FTS 主題詞比對)不是合法的「檢索建池」路徑。** 它是「對已落地書目的全文字串比對」,召回受限於「本地庫剛好有什麼」——**無 IPC 錨定、無官方來源梯、無 provenance 稽核、無家族去重**的裸關鍵字匹配。用它建池 = 把檢索母體從「三地官方專利庫」偷偷縮成「本地 sqlite 碰巧存的子集」,**召回洞無法診斷(精雕迴圈④失效)、precision 也是假的**——比鐵律0「母數≠樣本」更惡劣:連母體都是污染的。

- **合法檢索建池入口只有 `patent_search`(relevance)/ `patent_bulk`(coverage)** ——走官方梯(GPSS→EPO→PPUBS),欄位化布林、IPC 錨定、命中即 absorb、provenance 可稽核。
- **`patentdb_query` FTS 的合法用途**:只用於「腳踏兩條船並查全域庫」(§3 step 4.5)——查跨案累積的**已落地**書目有無本案沒撈到、但適合的分析標的,命中者**併入** `candidates.csv`(標 `from_patentdb`)一起評分,**是線上檢索矩陣的補充,不取代、不作為建池主體**。
- **自檢**:「這個池是用 `patent_search`/`patent_bulk` 官方檢索式建的,還是用本地 FTS 撈的?」後者 = 方法論污染,整池作廢,回主幹第 3 步用 ABCD 官方檢索式重建。

### 1.4.5 ⚠️ `patent_bulk(source=gpss)` 的 B 群引號片語陷阱（跨後端 total 不一致）

> **實撈假零命中≠真零命中。** `gpss4_advanced_search`（進階檢索、零額度計數）與 `patent_bulk(source=gpss)`（records 實撈）是 **兩個不同後端**，對多字帶連字號的引號片語（`"test-time adaptation"`、`"continual learning"`、`"rule engine"`）解析不一致。

- **症狀**：同一運算式，`gpss4_advanced_search` 計數非零（例：78），但 `patent_bulk` 回 `total=0 / exhausted`。bulk 路徑把整組引號片語吞成零命中。
- **修正**：把 B 群拆成**單 token 主幹**（`few-shot`/`continual`/`incremental`，必要時保留一個短引號片語如 `"test-time"`），即正常命中。單字覆蓋量 > 引號片語的精確痛點，於粗池階段可接受（後續去噪工序會收緊）。
- **鐵律**：`patent_bulk(source=gpss)` 的 B/C 群**避免長引號片語**，優先單 token。若 `gpss4_advanced_search` 計數非零而 bulk 回零，**以能命中的路徑為準、拆詞重撈、於 journal 記差異**，勿當真零命中放棄（否則將造成召回洞）。
