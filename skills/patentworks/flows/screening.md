...
3. **檢索一律用單一入口 `patent_search`**——來源梯已內建於 server(TIPO GPSS 官方首選 → EPO → USPTO PPUBS → gated 爬蟲),依憑證與查詢軸自動路由,每級嘗試記入 `provenance`;爬蟲尾級須 `allow_scraping=True`(使用者明確同意),否則官方全 miss 即回 `SCRAPING_REQUIRED`。`build_screening_table` 內部同樣走 dispatcher(也有 `allow_scraping=False` 參數)。單號取文/取圖依 `../SKILL.md` §5 各級能力知識選工具。
...
5. **交付**:寫回後的 CSV 即交付物 → `stage_file(path)` 回 handle(或回傳 build 時的同一 handle)。相關 PDF 用 `fetch_patent_pdf`(官方路由優先,預設 `allow_scraping=False`);代表圖走 `extract_representative_figure`(TW 案優先 `gpss_download_representative_figure`,需同意)。`gpatents_download_*` 僅為最後手段。給使用者的「答案」是白話綜述 + handle,不是貼表。
