...
1. **最終交付一律是人類可讀的表格(xlsx)**,經 token+download_url handle 交付。
...
## 流程
1. **召回 + 落地(一個工具)**:呼叫 patentmcp **`build_screening_table(cpc, keyword, purpose, ...)`**。它在 server 端 search→家族去重→切 Claim1→**寫成 xlsx 落地 token store**,只回 `{handle, count, deduped, source, columns, gaps}`——**原始候選列不進 context**。
...
2. **讀表消化(agent 端)**:用 xlsx skill **分批讀**(每批 ~30 列的摘要+獨立項),不一次灌進 context。每列判讀後**寫回同一 xlsx**:`相關性`(相關/可能/否)、`分數`(H/M/L)、`技術要點`(1–2 句,用本案語彙)、`命中/落差要件`、`理由`。
3. **深讀(僅 shortlist)**:需要時 `gpatents_get` 取完整 claims 做要件對照。
4. **可專利性綜述(若是該子情境)**:彙整最接近前案 + 要件覆蓋 → **你的差異點**。
5. **交付**:寫回後的 xlsx 即交付物 → `stage_file(path)` 回 handle(或回傳 build 時的同一 handle)。相關 PDF/代表圖用 `gpatents_download_pdf/figure` 取 handle。給使用者的「答案」是白話綜述 + handle,不是貼表。
